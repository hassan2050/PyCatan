...
class CatanLog(object):
    ...
    def log_player_roll(self, player, roll):
        ...
        self._logln('{0} rolls {1}'.format(player.color, roll))
    ...
    def log_player_moves_robber_and_steals(self, player, location, victim):
        ...
        """
        if victim is None:
          self._logln('{0} moves robber to {1}'.format(
              player.color,
              location
          ))
        else:
    ...
    def log_player_trades_with_bank(self, player, to_bank, to_player):
        ...
        # to_port items
        self._log('[')
        for i, res in enumerate(to_bank):
            if i > 0:
                self._log(', ')
            self._log('{0}'.format(res))
        self._log(']')

        self._log(' to bank for ')

        self._log("{0}".format(to_player))
